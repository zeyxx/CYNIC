...
**Clippy test debt:** `cargo clippy --all-targets` mesure actuelle 2026-04-16 post-3 classes extinguées : **0 errors + 10 warnings**. Classes éliminées : `manual_repeat` (5 sites), `vec_init_then_push` (compliance.rs:252 — workaround ICE nightly stale confirmée obsolète sur stable 1.94.1), `absurd_extreme_comparisons` (probes/network.rs:189 — assert tautologique sur u64 remplacé par smoke test propre). Remaining warnings (all style/test) : `print_stdout` x7 (`tests/calibration_token.rs:351-357`), `uninlined_format_args` x2 (`tests/storage_contract.rs:226,445`), `eprintln!` x1 (`src/backends/cli.rs:157`, test). **`--all-targets -D correctness -D perf` passe** ; `-D warnings` encore rouge sur style. Pre-commit gate gap (R20 latent) : `cargo clippy --lib` ne couvre pas `--all-targets`.
