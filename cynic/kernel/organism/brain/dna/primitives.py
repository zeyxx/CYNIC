...
class DNA_Cell:
    ...
    def state_key(self) -> str:
        """Generate a state key for learning (matching core Cell logic)."""
        reality = self.source.upper()
        analysis = self.metadata.get("analysis", "JUDGE").upper()
        time_dim = self.metadata.get("time_dim", "PRESENT").upper()
        lod = self.metadata.get("lod", 1)
        return f"{reality}:{analysis}:{time_dim}:{lod}"
...
class DNA_Judgment:
    ...
    def from_judgment(cls, judgment: Any) -> DNA_Judgment:
        ...
        return cls(
            ...
            dogs_votes=getattr(judgment, "dog_votes", {}) or {},
...
async def JUDGE(
    ...
    judgment = await orchestrator.run(cynic_cell, level=level_enum)
