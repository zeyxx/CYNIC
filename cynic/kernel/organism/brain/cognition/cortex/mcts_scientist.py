...
logger = logging.getLogger(
    "cynic.kernel.organism.brain.cognition.cortex.mcts_scientist"
)
...
class ScientificMCTS:
    ...
    def add_hypothesis(self, parent_id: str, hypothesis: Hypothesis) -> str:
        ...
        node = ExperimentNode(
            id=hypothesis.id, parent_id=parent_id, hypothesis=hypothesis
        )
    ...
    def select_next_experiment(self) -> Optional[ExperimentNode]:
        ...
        if current.status == "SUCCESS":
            logger.info(
                f"[MCTS] Node {current.id} is a successful leaf. Needs expansion."
            )
    ...
    def backpropagate(self, node_id: str, q_score: float, status: str):
        ...
        logger.info(
            f"[MCTS] Backpropagated Q={q_score:.2f} from {node_id}. Status: {status}"
        )
