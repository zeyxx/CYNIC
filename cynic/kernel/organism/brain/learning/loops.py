...
class SONA:
    ...
    async def _on_judgment(self, event: Event) -> None:
        ...
        await self._trigger_judgment_learning(event.dict_payload)
    ...
    async def _on_ewc_checkpoint(self, event: Event) -> None:
        ...
        logger.info("EWC checkpoint received - consolidating %s", event.dict_payload)
    ...
    async def _aggregate_learning(self, payload: dict[str, Any]) -> None:
        ...
        sum(config.priority for config in self.loops.values() if config.enabled)
