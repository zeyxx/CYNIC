...
**Clippy debt — CLOSED 2026-04-16.** `cargo clippy -p cynic-kernel --all-targets -- -D warnings` passe clean (0 errors + 0 warnings). Classes extinguées sur 3 sessions successives : `manual_repeat` (5 sites), `vec_init_then_push` (workaround ICE nightly stale), `absurd_extreme_comparisons` (assert tautologique), `print_stdout` x7 (K12-allow sur `print_corpus` — diagnostic --nocapture), `print_stderr` x1 (K12-allow sur test `#[ignore]`d gemini CLI SKIP notice), `uninlined_format_args` x2 (inline `{VAR}`). **Reste ouvert** : R20 latent — pre-commit hook utilise `cargo clippy --lib` alors que `--all-targets -D warnings` passe maintenant ; promouvoir le gate dans `.claude/rules/workflow.md` + `Makefile` pour que le pre-commit couvre tests et bibs.
