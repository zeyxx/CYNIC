...
class JudgeOrchestrator:
    ...
    async def run(
        ...
        if selected_level is None:
            selected_level = await self._select_level(cell, budget_usd or cell.budget_usd)
        ...
        try:
            ...
            if self.escore_tracker:
                for dog_id, score in judgment.dog_votes.items():
                    self.escore_tracker.update_dimension(f"agent:{dog_id}", "JUDGE", score / 100.0)

            # Generate reasoning if missing
            if not judgment.reasoning:
                explanation = await self._dialogue_agent.explain_judgment(judgment, question=str(cell.content))
