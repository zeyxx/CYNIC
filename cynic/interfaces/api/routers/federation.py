...
async def get_federation_status(container: AppContainer = Depends(get_app_container)):
    ...
    if not container.organism.gossip_manager:
        raise HTTPException(status_code=404, detail="Federation not enabled on this instance")
    return container.organism.gossip_manager.get_stats()

@router.post("/peers")
async def add_peer(peer_id: str, url: str, container: AppContainer = Depends(get_app_container)):
