...
- [x] **K15 producer: verdicts → kernel /observe (2026-05-02 FULLY OPERATIONAL).** Wired k15_emitter.py into domain_verdict_builder.py + wallet_corpus_builder.py. Real-time emission on build completion. **Status:** FULLY OPERATIONAL (emit_observation returns True, observations stored in /observations). Fixed: (1) protocol handling (CYNIC_REST_ADDR host:port → http://host:port), (2) HTTP status 200/202 acceptance. Kernel LIVE, K15 pipeline E2E verified. **Commits:** b0f0a18 (wiring) + ed8f99b1 (emitter fixes). **Next:** run domain_verdict_builder + wallet_corpus_builder; verify K15 loop (emit → consume → organism learns).
- [ ] **K15 loop falsification: emit → consume → verify organism learns (May 2-3 EOD).** Run domain_verdict_builder to emit 5-10 verdict batches to /observe. Measure baseline axiom confidence via `/health` → verdicts_queued counter. Wait 24h. Re-measure axiom confidence. **Falsify:** ≥2 Dogs show ≥3% confidence lift after consuming emitted verdicts. **Milestone:** K15 proves organism learning loop is closed (no manual merges, data flows live).
