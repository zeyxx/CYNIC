...
class DogStatus(BaseModel):
    ...
    confidence: float | None = Field(
        ...
        ge=0.0,
        le=0.618,
        description="Confidence bound [0, 0.618] ()",
    )
...
class AccountStatusResponse(BaseModel):
    ...
    learn_rate: float = Field(
        ge=0.0,
        le=0.618,
        description="Learning rate [0, =0.618]",
    )
...
class AgentScore(BaseModel):
    ...
    total: float = Field(
        ge=0.0,
        le=100.0,
        description="Aggregate E-Score (-weighted geometric mean) [0, 100]",
    )
...
class PolicyAction(BaseModel):
    ...
    confidence: float = Field(
        ge=0.0,
        le=0.618,
        description="Confidence in prediction [0, =0.618]",
    )
...
class PolicyStatsResponse(BaseModel):
    ...
    average_confidence: float = Field(
        ge=0.0,
        le=0.618,
        description="Mean confidence across all learned actions [0, =0.618]",
    )
