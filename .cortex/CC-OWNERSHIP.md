...
- **Staleness signal:** `git log -1 --format=%cr -- .cortex/CC-OWNERSHIP.md` > 14 days = review
- **Falsification:** `diff <(jq -r '.hooks | to_entries[].value[].hooks[].command' .claude/settings.json | sort) <(ls .cortex/mcp/*.sh | sort)` — every wired hook has a script, every script is wired
