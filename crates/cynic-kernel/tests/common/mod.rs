...
pub(crate) async fn setup_test_db(suffix: &str) -> Option<SurrealHttpStorage> {
    let millis = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .unwrap()
        .as_millis();
    let db_name = format!("test_{millis}_{suffix}");
...
pub(crate) async fn teardown_test_db(db: &SurrealHttpStorage) {
    let _ = db
        .query(&format!("REMOVE DATABASE `{}`;", db.db_name()))
        .await;
}
