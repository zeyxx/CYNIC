...
| RC7-2 | Audit | Multi-hop correlation — CYNIC needs request tracing | PARTIAL — v0.7.4: request_id in PipelineDeps, generated by REST/MCP callers, propagated via tracing span. Remaining: #[instrument] on Dog evaluate() and storage calls for full multi-hop trace. |
