...
24. **Hook network calls are bounded.** Every `curl`, `gh`, `git fetch/push/pull` in a hook or session script MUST use `timeout N` or `--max-time`. Hooks run synchronously — one hung network call blocks all git operations or session init indefinitely. Seen 2026-05-16: `gh pr list` without timeout hung pre-push for minutes. — Falsify: `grep -rn "curl\|gh \|git fetch\|git push" scripts/git-hooks/ .claude/hooks/ | grep -v "timeout\|max-time\|max_time\|command -v\|echo"` — zero hits expected.
