...
|------|---------|----------|---------|----------|--------|--------|-------|
| 2026-04-17 | r23-gate (lint-subprocess-env + R21) | ~1h15 | 2 (6be59fc + 1fa7d38, PR #7 merged) | 0 | 1 (R23 meta-principle: rule → gate → falsification closed) | 0 | **Hypothèse :** R23 adoptée 2026-04-16 sans gate = violation meta-principle (Fix→Test→Gate→Verify) + #20/#21. **Mesure :** pre-gate = 0 real violations (audit shell+Rust). Post-gate : négative control clean (exit 0), 3 positive controls R23a/R23b/R23c catch (exit 1), exempt marker pass, `make test-gates` = **16/16 falsifiés** (13 existing + 3 nouveau). Pre-commit validation : fmt ✓, clippy workspace all-targets ✓, build --tests ✓. **Scope gate** : shell `cargo (build\|test\|clippy\|run\|check\|audit)` → `RUST_MIN_STACK` in file ; standalone `rustfmt` → `--edition` per line ; Rust `Command::new("cargo"\|"rustfmt")` → `.env(` / `"--edition"` in file. Exemption file-level `# R23-exempt:` / `// R23-exempt:`. Wired into `make check` chain (lint-drift → **lint-subprocess-env** → lint-security). Branch-PR flow OK : `feat/r23-gate-2026-04-17` → PR #7 → admin-merge → origin/main sync. **Pacing :** 1 item, stop. |
