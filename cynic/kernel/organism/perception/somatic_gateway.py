...
class SomaticGateway:
    ...
    def register_conduit(self, conduit: Conduit, reality: str = "INTERNAL"):
        ...
        logger.debug(
            f"[{self.instance_id}] Registered conduit: {conduit.conduit_id} (Reality: {reality})"
        )
    ...
    async def ingest(self, conduit_id: str, data: Any):
        ...
        logger.debug(
            f"[{self.instance_id}] Ingesting from {conduit_id} (Reality: {reality})"
        )
        ...
        try:
            ...
            if self._dropped_count % 100 == 0:
                logger.warning(
                    f"[{self.instance_id}] Somatic Gateway: Buffer full. Dropped {self._dropped_count} observations."
                )
    ...
    def _should_filter(self, reality: str, data: Any) -> bool:
        ...
        if now - last_time < min_interval:
            self._dropped_count += 1
            logger.debug(
                f"[{self.instance_id}] Filtered {reality} (Time: delta={now - last_time:.3f}s < {min_interval}s)"
            )
            return True
        ...
        try:
            ...
            if self._last_content_hash.get(reality) == content_hash:
                self._dropped_count += 1
                logger.debug(
                    f"[{self.instance_id}] Filtered {reality} (Deduplication: content matches)"
                )
                return True
    ...
    async def _emit(self, source: str, reality: str, data: Any):
        ...
        logger.info(
            f"[{self.instance_id}] Somatic Gateway: Emitting PERCEPTION_RECEIVED for {reality} from {source}"
        )
        ...
        await self.bus.emit(
            Event.typed(CoreEvent.PERCEPTION_RECEIVED, cell.model_dump(), source=source)
        )
    ...
    def stats(self) -> dict:
        ...
        return {
            ...
            "buffer_load": self._buffer.qsize() / self._buffer.maxsize
            if self._buffer.maxsize
            else 0,
        }
