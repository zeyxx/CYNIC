...
Scans cynic.brain.cognition.neurons for all AbstractDog subclasses with a DOG_ID class attribute.
...
import cynic.brain.cognition.neurons as dogs_pkg
from cynic.brain.cognition.neurons.base import AbstractDog, DogId

logger = logging.getLogger("cynic.brain.cognition.neurons.discovery")


def discover_dog_classes() -> dict[str, type[AbstractDog]]:
    """
    Scan cynic.brain.cognition.neurons for all concrete AbstractDog subclasses with DOG_ID.
    ...
    for _, module_name, _ in pkgutil.iter_modules(dogs_pkg.__path__):
        ...
        try:
            module = importlib.import_module(f"cynic.brain.cognition.neurons.{module_name}")
        except Exception:
            logger.warning("Failed to import cynic.brain.cognition.neurons.%s", module_name, exc_info=True)
            continue
