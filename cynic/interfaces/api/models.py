...
class JudgeRequest(BaseModel):
    ...
    fractal_depth: int = Field(
        ...
        ge=1,
        le=55,
        description="Recursion depth for axiom facets (1=default, 2+=fractal)",
    )
...
class AccountRequest(BaseModel):
    ...
    judgment_id: str = Field(
        default="", description="Optional: cost a specific judgment"
    )
    trigger_emerge: bool = Field(
        default=True, description="Trigger EMERGE pattern detection"
    )
...
class AccountResponse(BaseModel):
    ...
    emergence_pattern: str = Field(
        default="", description="Pattern type (SPIKE/RISING/STABLE_HIGH)"
    )
    message: str
...
class EcosystemStateResponse(BaseModel):
    ...
    automation_events: list[EventSnapshot] = Field(
        description="Scheduler + automation events"
    )
...
class DecisionTraceResponse(BaseModel):
    ...
    path: list[DecisionPathStage] = Field(
        default_factory=list, description="Path through guardrails"
    )
...
class TopologyConsciousnessResponse(BaseModel):
    ...
    topology_deltas_computed: int = Field(
        description="Count of computed topology deltas"
    )
    ...
    recent_changes: list[dict[str, Any]] = Field(
        default_factory=list, description="Recent topology changes"
    )
    timestamp: float = Field(description="Response timestamp")
...
class GuardrailDecision(BaseModel):
    ...
    guardrail_type: str = Field(
        description="Type of guardrail (power_limiter, alignment, etc)"
    )
...
class NervousSystemAuditResponse(BaseModel):
    ...
    decision_reasons: list[dict[str, Any]] = Field(
        default_factory=list, description="Decision traces (why was action taken)"
    )
    loop_integrity_checks: list[dict[str, Any]] = Field(
        default_factory=list, description="Loop closure validation checks"
    )
...
class SelfAwarenessResponse(BaseModel):
    ...
    improvement_proposals: list[Any] = Field(
        default_factory=list, description="Proposed improvements to self"
    )
