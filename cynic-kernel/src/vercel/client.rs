...
use reqwest::Client;
use super::types::{VercelDeployment, CreateDeploymentRequest, VercelError};
...
impl VercelClient {
    ...
    pub async fn list_deployments(&self, app_name: &str) -> Result<Vec<VercelDeployment>, VercelError> {
        ...
        let res = self.client
            ...
            .send()
            .await
            .map_err(|e| VercelError { code: "network_error".to_string(), message: e.to_string() })?;
    ...
    pub async fn create_deployment(&self, request: CreateDeploymentRequest) -> Result<VercelDeployment, VercelError> {
