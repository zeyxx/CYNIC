...
class CoreEvent(StrEnum):
    ...
    CONFIGURATION_MUTATED = "core.config_mutated"  # Self-tuning event
...
class EventBus:
    ...
    async def emit(self, event: Event) -> None:
        ...
        for h in all_handlers:
            task = asyncio.create_task(
                self._run_handler(h, event), name=f"bus_{self.bus_id}_{event.type}"
            )
    ...
    async def _run_handler(self, handler: Handler, event: Event) -> None:
        try:
            ...
            self._error_count += 1
            logger.error(
                "Handler error on bus %s [%s]: %s", self.bus_id, event.type, e, exc_info=True
            )
    ...
    async def drain(self, timeout: float = 2.0) -> None:
        ...
        try:
            await asyncio.wait_for(
                asyncio.gather(*self._pending_tasks, return_exceptions=True), timeout=timeout
            )
        except TimeoutError:
