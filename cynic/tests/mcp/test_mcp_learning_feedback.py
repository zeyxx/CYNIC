...
from cynic.mcp.router import MCPRouter, _jsonrpc_result, _INTERNAL_ERROR
from cynic.mcp.models import LearnRequest, FeedbackSignal
...
async def test_ask_cynic_returns_judgment_result():
    ...
    with patch("cynic.mcp.router.get_app_container", return_value=mock_container):
        message = {
            "jsonrpc": "2.0",
            "id": 1,
            "method": "tools/call",
...
async def test_learn_endpoint_emits_user_feedback_event(monkeypatch):
    ...
    # Mock event bus
    mock_bus = AsyncMock()
    monkeypatch.setattr("cynic.mcp.server.get_core_bus", lambda: mock_bus)
...
async def test_mcp_tool_called_event_emitted_before_handler(monkeypatch):
    ...
    monkeypatch.setattr("cynic.mcp.router.get_app_container", lambda: mock_container)
    monkeypatch.setattr("cynic.mcp.router.get_core_bus", lambda: mock_bus)
