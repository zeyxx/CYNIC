...
async def run_reflection_loop():
    ...
    analyst = next((a for a in available if "coder" in a.adapter_id.lower()), available[0])
    sage = next((a for a in available if "deepseek" in a.adapter_id.lower()), available[-1])
    ...
    analyst_report = await get_diagnosis(analyst, "ANALYST (Syntax & Logic)", truncated_content, target_file.name)
    print("-" * 40)
    print(analyst_report)
    print("-" * 40)
    
    sage_report = await get_diagnosis(sage, "SAGE (Architecture & Phi)", truncated_content, target_file.name)
    print("-" * 40)
    print(sage_report)
    print("-" * 40)
