...
CREATE INDEX IF NOT EXISTS idx_td_error_service ON td_error_tracker(service_id);
CREATE INDEX IF NOT EXISTS idx_td_error_convergence ON td_error_tracker(is_converged, rolling_avg_td_error);
CREATE INDEX IF NOT EXISTS idx_td_error_drift ON td_error_tracker(is_drift, created_at);
CREATE INDEX IF NOT EXISTS idx_td_error_state_action ON td_error_tracker(state, action);
CREATE INDEX IF NOT EXISTS idx_td_error_created ON td_error_tracker(created_at DESC);
...
INSERT INTO _migrations (name) VALUES ('042_td_error_tracker')
