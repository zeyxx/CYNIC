...
try:
    ...
    print("  [PR Reviews]")
    print(
        f"    - Approvals required: {protection_data['required_pull_request_reviews']['required_approving_review_count']}"
    )
    print(
        f"    - Dismiss stale reviews: {protection_data['required_pull_request_reviews']['dismiss_stale_reviews']}"
    )
    print()
    print("  [Enforcement]")
    print(
        f"    - Strict mode (up to date): {protection_data['required_status_checks']['strict']}"
    )
    print(f"    - Linear history: {protection_data['required_linear_history']}")
    print(
        f"    - Force pushes: {'BLOCKED' if not protection_data['allow_force_pushes'] else 'ALLOWED'}"
    )
    print(
        f"    - Deletions: {'BLOCKED' if not protection_data['allow_deletions'] else 'ALLOWED'}"
    )
    print()
