...
2. **Observer pipeline check:**
   ...
   [ -x "$CLAUDE_PROJECT_DIR"/.claude/hooks/observe-tool.sh ] && OBS_TOOL="yes"
   [ -x "$CLAUDE_PROJECT_DIR"/.claude/hooks/observe-prompt.sh ] && OBS_PROMPT="yes"
   [ -x "$CLAUDE_PROJECT_DIR"/.claude/hooks/observe-subagent.sh ] && OBS_AGENT="yes"
   echo "  tool=$OBS_TOOL  prompt=$OBS_PROMPT  subagent=$OBS_AGENT"
   # Check if observe-tool.sh has domain/tags (v2 indicator)
   grep -q 'DOMAIN=' "$CLAUDE_PROJECT_DIR"/.claude/hooks/observe-tool.sh 2>/dev/null && echo "  observe-tool: v2 (domain+tags)" || echo "  observe-tool: v1 (basic)"
   ```
