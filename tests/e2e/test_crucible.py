...
async def test_crucible_lifecycle():
    ...
    try:
        ...
        organism.senses.somatic_gateway.register_conduit(
            mock_conduit, reality="GAMBLING"
        )
        ...
        while time.time() - start_wait < timeout:
            if (
                CoreEvent.LEARNING_EVENT.value in events_seen
                or "core.learning_event" in events_seen
            ):
                found_learning = True
                break
        ...
        assert (
            CoreEvent.PERCEPTION_RECEIVED.value in events_seen
        ), "Perception never reached the bus"
        assert (
            CoreEvent.JUDGMENT_CREATED.value in events_seen
        ), "Cortex never judged the stimulus"
