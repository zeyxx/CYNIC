...
**Clippy test debt:** `cargo clippy --all-targets` rouge. Mesure 2026-04-16 post-`manual_repeat` fix : **2 errors + 10 warnings** (was 3e+13w). Classe `manual_repeat` extinguée (5 sites : 3 rest_routes.rs + 2 data.rs, grep=0). Remaining classes : `print_stdout` x7 (`tests/calibration_token.rs:351-357`), `uninlined_format_args` x2 (`tests/storage_contract.rs:226,445`), `absurd_extreme_comparisons` x1 (`src/infra/probes/network.rs:189`, `rx_bytes <= u64::MAX` toujours vrai), `eprintln!` x1 (`src/backends/cli.rs:157`, test), `vec_init_then_push` x1 (`src/domain/compliance.rs:252`, préfère `vec![...]`). Les 2 errors restantes sont en code prod (classes correctness/perf, pas tests). Pre-commit gate gap observed : `cargo clippy --lib -- -D warnings` clean alors que `--all-targets` échoue — R20 latent (gate ne DIT pas ce qu'il claim).
