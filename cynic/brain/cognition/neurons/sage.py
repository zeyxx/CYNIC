...
from cynic.kernel.core.axioms import DEFAULT_CONTEXTUAL_WEIGHTS
from cynic.kernel.core.consciousness import ConsciousnessLevel
from cynic.kernel.core.judgment import Cell
from cynic.brain.cognition.neurons.base import (
    LLMDog, DogCapabilities, DogHealth, DogJudgment,
    DogId, HealthStatus,
)

logger = logging.getLogger("cynic.brain.cognition.neurons.sage")
...
class SageDog(LLMDog):
    ...
    async def dream_facets(
        ...
        """
        World-Maker: Generate 7 dynamic facets for an axiom in a new reality.
        
        This is the 'Faiseur de Mondes' logic.
        """
        ...
        try:
            ...
            for line in response.splitlines():
                if ":" in line:
                    ...
                    desc = parts[1].strip()
                    # Clean up name (remove numbering etc)
            ...
            logger.error(f"SAGE failed to dream facets for {axiom} in {reality}: {e}")
            return {}
    ...
    async def _temporal_path(
        ...
        """
        from cynic.brain.llm.temporal import temporal_judgment, fast_temporal_judgment

        content = text or cell.context or ""
        # Inject temporal dimension into context
        temporal_context = f"Temporal Dimension: {cell.time_dim}\n{compressed_context}"
        
        if lod_level == 1:
            # R2: REDUCED LOD — organism under resource pressure → 3-perspective fast path
            tj = await fast_temporal_judgment(adapter, content, context=temporal_context)
            perspective_count = 3
        else:
            tj = await temporal_judgment(adapter, content, context=temporal_context)
            perspective_count = 7
        ...
        reasoning = (
            f"*sniff* Chokmah ({cell.time_dim}/{perspective_count}p): Q={q_score:.1f}. "
