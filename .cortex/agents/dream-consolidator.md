...
1. Read the file
2. Classify: **keep** (still relevant), **merge** (overlaps with another), **delete** (stale, superseded, already in .cortex/rules/)
...
- Feedback that restates a rule in `.cortex/rules/` without adding WHY → delete
...
```bash
STATE_FILE="$HOME/.cortex/projects/-home-user-Bureau-CYNIC/memory/.dream-state"
