...
impl TaskHealth {
    ...
    pub fn touch_summarizer(&self, detail: &'static str) {
        ...
        if let Ok(mut d) = self.summarizer_detail.write() { *d = detail; }
    }

    /// CCM aggregate: HONEST touch with detail.
    pub fn touch_ccm_aggregate(&self, detail: &'static str) {
        self.ccm_aggregate.store(Self::now_secs(), Ordering::Relaxed);
        if let Ok(mut d) = self.ccm_aggregate_detail.write() { *d = detail; }
    }
    ...
    pub fn touch_backfill(&self, detail: &'static str) {
        ...
        if let Ok(mut d) = self.backfill_detail.write() { *d = detail; }
    }

    /// Snapshot of all task health — for /health endpoint.
    pub fn snapshot(&self) -> Vec<TaskSnapshot> {
        let now = Self::now_secs();
        let sum_detail = self.summarizer_detail.read().map(|d| *d).unwrap_or("unknown");
        let ccm_detail = self.ccm_aggregate_detail.read().map(|d| *d).unwrap_or("unknown");
        let bf_detail = self.backfill_detail.read().map(|d| *d).unwrap_or("unknown");
        vec![
            TaskSnapshot::new("coord_expiry", self.coord_expiry.load(Ordering::Relaxed), now, 120, None),
            TaskSnapshot::new("usage_flush", self.usage_flush.load(Ordering::Relaxed), now, 120, None),
            TaskSnapshot::new("ccm_aggregate", self.ccm_aggregate.load(Ordering::Relaxed), now, 600, Some(ccm_detail)),
            TaskSnapshot::new("summarizer", self.summarizer.load(Ordering::Relaxed), now, 1200, Some(sum_detail)),
            TaskSnapshot::new("rate_eviction", self.rate_eviction.load(Ordering::Relaxed), now, 120, None),
            TaskSnapshot::new("health_loop", self.health_loop.load(Ordering::Relaxed), now, 60, None),
            TaskSnapshot::new("remediation", self.remediation.load(Ordering::Relaxed), now, 60, None),
            TaskSnapshot::new("introspection", self.introspection.load(Ordering::Relaxed), now, 600, None),
            TaskSnapshot::new("backfill", self.backfill.load(Ordering::Relaxed), now, 600, Some(bf_detail)),
        ]
    }
}
...
impl TaskSnapshot {
    fn new(name: &'static str, last: u64, now: u64, expected_interval: u64, detail: Option<&'static str>) -> Self {
        if last == 0 {
            Self { name, last_success_ago: 0, status: "never", detail }
        } else {
            let ago = now.saturating_sub(last);
            let status = if ago <= expected_interval * 2 { "ok" } else { "stale" };
            Self { name, last_success_ago: ago, status, detail }
        }
    }
}
...
mod tests {
    ...
    fn summarizer_detail_tracks_llm_state() {
        ...
        let snap = th.snapshot();
        let sum = snap.iter().find(|s| s.name == "summarizer").expect("summarizer");
    ...
    fn ccm_aggregate_detail_tracks_idle() {
        ...
        let snap = th.snapshot();
        let ccm = snap.iter().find(|s| s.name == "ccm_aggregate").expect("ccm");
    ...
    fn backfill_visible_in_snapshot() {
        ...
        let snap = th.snapshot();
        let bf = snap.iter().find(|s| s.name == "backfill").expect("backfill");
