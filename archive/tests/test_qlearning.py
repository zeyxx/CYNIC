...
def test_qtable_creates_entry_on_first_update(
    qtable: QTable, learning_signal: LearningSignal
) -> None:
...
def test_matrix_stats_coverage(qtable: QTable) -> None:
    ...
    # Add some states
    qtable.update(
        LearningSignal(state_key="CODE:JUDGE:PRESENT:1", action="WAG", reward=0.5)
    )
    qtable.update(
        LearningSignal(state_key="CODE:JUDGE:FUTURE:2", action="WAG", reward=0.5)
    )
    qtable.update(
        LearningSignal(state_key="MARKET:ANALYSIS:PAST:1", action="WAG", reward=0.5)
    )
...
def test_top_states_returns_sorted(qtable: QTable) -> None:
    ...
    for _ in range(10):
        qtable.update(
            LearningSignal(state_key="high:visit:1", action="WAG", reward=0.5)
        )
