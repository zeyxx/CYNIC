...
fn parse_token_metrics(content: &str) -> Option<TokenMetrics> {
...
fn score_token_analysis(m: &TokenMetrics) -> AxiomScores {
...
impl Dog for DeterministicDog {
    ...
    async fn evaluate(&self, stimulus: &Stimulus) -> Result<AxiomScores, DogError> {
        ...
        if let Some(ref token_metrics) = parse_token_metrics(content) {
            return Ok(score_token_analysis(token_metrics));
        }
...
mod tests {
    ...
    // ── Token-analysis domain scoring tests ────────────────────
    ...
    fn parse_token_metrics_healthy_token() {
        ...
        let m = parse_token_metrics(&content).expect("should parse");
    ...
    fn parse_token_metrics_rejects_non_token() {
        assert!(parse_token_metrics("Just a normal sentence.").is_none());
        assert!(parse_token_metrics("[DOMAIN: chess]\n[METRICS]\n").is_none());
    }
    ...
    async fn token_healthy_scores_high() {
        ...
        // Healthy token: all axioms should score well above NEUTRAL
        assert!(
        ...
        // Zero abstentions — all axioms judged
        assert!(
    ...
    async fn token_rug_scores_low() {
        ...
        // Rug token: all axioms should score low
        assert!(
        ...
        // q_score should be in BARK range
    ...
    async fn token_moderate_scores_middle() {
        ...
        // Moderate token: scores should be between rug and healthy
        assert!(
    ...
    async fn token_scores_vary_across_profiles() {
        ...
        let sc_h = dog.evaluate(&s_healthy).await.unwrap();
        let sc_r = dog.evaluate(&s_rug).await.unwrap();

        // Every axiom should differ between healthy and rug
        assert!(
    ...
    async fn token_pump_fun_penalizes_culture() {
        ...
        let sc_pump = dog.evaluate(&s_pump).await.unwrap();
        let sc_manual = dog.evaluate(&s_manual).await.unwrap();
    ...
    async fn token_fallback_to_form_for_non_token() {
        let dog = DeterministicDog;
        // Non-token content should use FORM scoring (with abstentions)
        let stimulus = Stimulus {
            content: "A simple neutral statement about the world.".into(),
            context: None,
            domain: None,
        ...
        let scores = dog.evaluate(&stimulus).await.unwrap();
        // FORM scoring has abstentions on fidelity, verify, culture
        assert!(
            scores.abstentions.contains(&"culture".to_string()),
            "non-token should still use FORM scoring with abstentions"
        );
    }
}
