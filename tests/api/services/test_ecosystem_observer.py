...
def observer(journal, decision_trace, registry):
    ...
    return EcosystemObserver(
        journal=journal, decision_trace=decision_trace, handlers_registry=registry
    )
...
async def test_handler_traces_query(observer, decision_trace):
    ...
    # Start a trace
    trace_id = await decision_trace.start_trace(
        judgment_id="j_123", initial_phase="PERCEIVE"
    )
