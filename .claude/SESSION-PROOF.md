...
**Location:** `.claude/session-proof.json` (git-tracked, updated by hooks)
...
1. **Reads** `.claude/session-proof.json` (if it exists from prior session)
...
⚠ GIT STATE VIOLATIONS (Rule MC2/MC5 — coordinate before proceeding):
  ...
  → Review .claude/session-proof.json for previous session state
...
4. **No immutable chain yet** — currently `.claude/session-proof.json` is a single file, overwritten by each session. Future: append-only `.jsonl` or kernel-stored ledger.
