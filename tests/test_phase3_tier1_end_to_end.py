...
class TestPhase3Tier1EndToEnd:
    ...
    async def test_judge_full_cycle_reflex_level(self):
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            # 1. POST
            post_resp = await client.post("/api/judge", json={
    ...
    async def test_perceive_full_cycle(self):
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            post_resp = await client.post("/api/perceive", json={
...
class TestPhase3ResponseModels:
    ...
    async def test_judge_response_schema(self):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            ...
            data = resp.json()
            required = ["judgment_id", "q_score", "verdict", "confidence", "axiom_scores", "dog_votes", "consensus_reached", "consensus_votes"]
