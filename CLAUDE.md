...
32. **No cross-layer type leakage.** Application services (`pipeline.rs`, `judge.rs`) never import from delivery layers (`api/`). Domain port signatures never expose infra types (`serde_json::Value`, `CircuitBreaker`). Event types used by multiple layers belong in `domain/`. Falsifiable: `grep 'use crate::api' pipeline.rs judge.rs` — must return zero. `grep 'serde_json::Value' domain/` — must return zero. Measured: 3 violations found (KernelEvent in api::rest, serde_json in StoragePort, CircuitBreaker in Judge) — all are v0.7 gates.
