...
def generate_fractal_tensors(n_depth: int = 6, branching_factor: int = 7, device="cpu"):
    ...
    if mb_per_tensor > 2000:
        print(
            "⚠️ AVERTISSEMENT : Ce tenseur risque de saturer la VRAM d'un GPU standard."
        )
    ...
    flat_values = torch.empty(
        total_leaves, dtype=torch.float32, device=device
    ).uniform_(1.0, 100.0)
...
if __name__ == "__main__":
    ...
    print("[RÉSULTATS DE LA RÉDUCTION PYTORCH TENSORISÉE]")
    ...
    print(
        "Au lieu de faire 3 passes mémoire (log -> mult -> sum), Helion fera TOUT en 1 seule passe dans la SRAM du GPU."
    )
    print(
        "Attendez-vous à un Speedup additionnel de 2x à 5x par rapport à cette baseline PyTorch pure !"
    )
