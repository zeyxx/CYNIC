...
async fn ping_succeeds_on_healthy_db() {
    let Some(db) = common::setup_test_db("ping").await else {
        return;
    };
...
async fn verdict_store_and_retrieve() {
    let Some(db) = common::setup_test_db("verdict_rt").await else {
        return;
    };
    ...
    assert_eq!(
        got.dog_scores.len(),
        1,
        "dog_scores should persist and round-trip"
    );
...
async fn verdict_list_respects_limit() {
    let Some(db) = common::setup_test_db("verdict_list").await else {
        return;
    };
    for i in 0..5 {
        let v = test_verdict(&format!("v-{i:03}"));
...
async fn crystal_observe_creates_and_updates() {
    let Some(db) = common::setup_test_db("crystal_obs").await else {
        return;
    };
    ...
    assert!(
        ...
        "confidence should increase with higher score"
    );
...
async fn observation_store_and_query() {
    let Some(db) = common::setup_test_db("obs_rt").await else {
        return;
    };

    db.store_observation(&test_obs("agent-1", "Edit", "main.rs"))
        .await
        .expect("store failed");
    db.store_observation(&test_obs("agent-1", "Edit", "main.rs"))
        .await
        .expect("store2 failed");
    db.store_observation(&test_obs("agent-1", "Read", "lib.rs"))
        .await
        .expect("store3 failed");

    let rows = db
        .query_observations("CYNIC", None, 50)
        .await
        .expect("query failed");
    ...
    assert!(
        ...
        "rows should have a non-empty 'target' field"
    );
...
async fn observation_query_session_targets() {
    let Some(db) = common::setup_test_db("obs_sess").await else {
        return;
    };

    // Two targets in same session
    db.store_observation(&test_obs("session-A", "Edit", "file1.rs"))
        .await
        .unwrap();
    db.store_observation(&test_obs("session-A", "Edit", "file2.rs"))
        .await
        .unwrap();
    // One target in different session
    db.store_observation(&test_obs("session-B", "Edit", "file3.rs"))
        .await
        .unwrap();

    let rows = db
        .query_session_targets("CYNIC", 500)
        .await
        .expect("query_session_targets failed");
    ...
    assert!(
        !rows.is_empty(),
        "session-A with 2 targets should produce co-occurrence data"
    );
    for row in &rows {
        assert!(
            !row.session_id.is_empty() || !row.target.is_empty(),
...
async fn coord_full_lifecycle() {
    let Some(db) = common::setup_test_db("coord_life").await else {
        return;
    };

    db.register_agent("test-agent", "claude", "testing")
        .await
        .expect("register failed");
    db.heartbeat("test-agent").await.expect("heartbeat failed");

    let result = db
        .claim("test-agent", "main.rs", "file")
        .await
        .expect("claim failed");
    ...
    db.release("test-agent", Some("main.rs"))
        .await
        .expect("release failed");
    let snap2 = db.who(Some("test-agent")).await.expect("who after release");
    assert!(
        snap2.claims.is_empty(),
        "claims should be empty after release"
    );

    db.deactivate_agent("test-agent")
        .await
...
async fn coord_claim_conflict_detection() {
    let Some(db) = common::setup_test_db("coord_conflict").await else {
        return;
    };

    db.register_agent("agent-A", "claude", "working")
        .await
        .unwrap();
    db.register_agent("agent-B", "gemini", "working")
        .await
        .unwrap();
    ...
    assert!(
        matches!(r2, ClaimResult::Conflict(_)),
...
async fn coord_claim_batch() {
    let Some(db) = common::setup_test_db("coord_batch").await else {
        return;
    };
    db.register_agent("batch-agent", "claude", "batch test")
        .await
        .unwrap();
    ...
    let result = db
        .claim_batch("batch-agent", &targets, "file")
        .await
...
async fn coord_expire_stale_does_not_break() {
    let Some(db) = common::setup_test_db("coord_expiry").await else {
        return;
    };
    db.register_agent("stale-agent", "test", "will expire")
        .await
        .unwrap();
    db.claim("stale-agent", "old-file.rs", "file")
        .await
        .unwrap();
    ...
    assert!(
        ...
        "fresh session should survive expire_stale"
    );
...
async fn audit_store_and_query() {
    let Some(db) = common::setup_test_db("audit_rt").await else {
        return;
    };

    let details = serde_json::json!({"action": "test", "target": "file.rs"}).to_string();
    db.store_audit("cynic_judge", "test-agent", &details)
        .await
        .expect("store_audit failed");
    db.store_audit("cynic_infer", "test-agent", &details)
        .await
        .unwrap();

    let all = db
        .query_audit(None, None, 10)
        .await
        .expect("query_audit failed");
    assert!(
        all.len() >= 2,
...
async fn flush_usage_via_storage_port() {
    let Some(db) = common::setup_test_db("flush_port").await else {
        return;
    };
    ...
    let rows = db
        .query_one("SELECT * FROM dog_usage;")
        .await
...
async fn store_and_search_crystal_embedding() {
    let Some(db) = common::setup_test_db("embed_rt").await else {
        return;
    };
    ...
    embedding[1] = 0.1;
    embedding[2] = 0.3;
    db.store_crystal_embedding("chess-sicilian", &embedding)
        .await
        .expect("store_crystal_embedding failed");

    // Verify the embedding was stored
    let rows = db
        .query_one("SELECT id, array::len(embedding) AS dims FROM crystal WHERE embedding != NONE;")
        .await
        .expect("query failed");
    assert!(!rows.is_empty(), "crystal should have an embedding");
    assert_eq!(
        rows[0]["dims"].as_u64().unwrap_or(0),
        1024,
        "embedding should be 1024-dim"
    );
...
async fn semantic_search_returns_empty_when_no_embeddings() {
    let Some(db) = common::setup_test_db("knn_empty").await else {
        return;
    };

    let query = vec![0.1f32; 1024];
    let results = db
        .search_crystals_semantic(&query, 5)
        .await
        .expect("semantic search should not error on empty table");
    assert!(
        results.is_empty(),
        "should return empty when no crystals have embeddings"
    );
...
async fn semantic_search_round_trip() {
    ...
    emb_chess[0] = 0.9;
    emb_chess[1] = 0.8;
    db.store_crystal_embedding("chess-open", &emb_chess)
        .await
        .unwrap();
    ...
    emb_rust[500] = 0.9;
    emb_rust[501] = 0.8;
    db.store_crystal_embedding("code-rust", &emb_rust)
        .await
        .unwrap();
...
async fn store_and_list_session_summaries() {
    ...
    let Some(db) = common::setup_test_db("session_rt").await else {
        return;
    };
    ...
    db.store_session_summary(&s1)
        .await
        .expect("store s1 failed");
    db.store_session_summary(&s2)
        .await
        .expect("store s2 failed");

    let summaries = db.list_session_summaries(10).await.expect("list failed");
    assert_eq!(
        summaries.len(),
        2,
        "should have 2 summaries, got {}",
        summaries.len()
    );
...
async fn get_unsummarized_sessions() {
    let Some(db) = common::setup_test_db("unsum_rt").await else {
        return;
    };

    // Create observations for 2 agents
    for i in 0..5 {
        db.store_observation(&test_obs("agent-A", "Edit", &format!("file{i}.rs")))
            .await
            .unwrap();
    }
    for i in 0..3 {
        db.store_observation(&test_obs("agent-B", "Read", &format!("doc{i}.md")))
            .await
            .unwrap();
    }
    db.store_observation(&test_obs("agent-C", "Bash", "ls"))
        .await
        .unwrap();

    let pending = db
        .get_unsummarized_sessions(3, 10)
        .await
        .expect("get_unsummarized_sessions failed");
    let ids: Vec<&str> = pending.iter().map(|(id, _, _)| id.as_str()).collect();
    assert!(
        ids.contains(&"agent-A"),
        "agent-A (5 obs) should be pending"
    );
    assert!(
        ids.contains(&"agent-B"),
        "agent-B (3 obs) should be pending"
    );
    ...
    assert!(
        !ids2.contains(&"agent-A"),
        "agent-A excluded after summarization"
    );
...
async fn get_session_observations_returns_filtered() {
    let Some(db) = common::setup_test_db("sessobs_rt").await else {
        return;
    };

    db.store_observation(&test_obs("sess-X", "Edit", "a.rs"))
        .await
        .unwrap();
    db.store_observation(&test_obs("sess-X", "Bash", "cargo test"))
        .await
        .unwrap();
    db.store_observation(&test_obs("sess-X", "Read", "b.rs"))
        .await
        .unwrap();
    db.store_observation(&test_obs("sess-Y", "Edit", "other.rs"))
        .await
        .unwrap();

    let obs = db
        .get_session_observations("sess-X")
        .await
        .expect("get_session_observations failed");
    assert_eq!(
        obs.len(),
        3,
        "should have 3 observations for sess-X, got {}",
        obs.len()
    );
...
// NOTE: store_infra_snapshot, list_infra_snapshots, cleanup_infra_snapshots were removed
// (no acting consumer, K15 violation). Test removed 2026-04-13.
