"""
CYNIC Temporal MCTS Benchmark (3)

Measures the convergence speedup of Temporal MCTS (7 -weighted perspectives)
vs Standard MCTS (single perspective, uniform weight).

Hypothesis: Temporal MCTS converges  faster than Standard MCTS
  - Standard: needs ~800 iterations to find optimal (high-variance estimates)
  - Temporal:  needs ~250 iterations (3.2- faster due to -weighted averaging)

Key Mechanism:
  Standard MCTS: each node visit = 1 noisy sample ' high per-node variance
  Temporal MCTS: each node visit = 7 -weighted samples ' lower variance,
...
Design (no LLM, no DB " pure in-memory simulation):
  ...
  Temporal scoring: 7 -weighted perspective samples (past/present/future/etc)
  BenchmarkResult: convergence iteration + quality + speedup ratio

Usage:
  bench = MCTSBenchmark()
  result = bench.run(max_iterations=1000, seed=42)
  assert result.speedup_ratio >= 1.5  # Expect   improvement
"""
...
_TEMPORAL_WEIGHTS: dict[str, float] = {
    ...
    "PRESENT": 1.0,  #  = 1.000
    "PAST": PHI_INV,  # = 0.618
    "CYCLES": PHI_INV,  # = 0.618
    "FLOW": PHI_INV_2,  # = 0.382
    "NEVER": PHI_INV_2,  # = 0.382 (inverted: high = safe)
}
...
class SearchProblem:
    ...
    def temporal_sample(self, action: int) -> float:
        """
        7 -weighted perspective samples ' lower variance estimate.
        ...
        -weighted geometric mean reduces variance vs single sample.
        """
        true_q = self.true_value(action)
        log_sum = 0.0
...
class MCTSBenchmark:
    ...
    temporal MCTS achieves   speedup (the architectural hypothesis).
    """
    ...
    def run_multi(
        ...
        return {
            ...
            "phi2_hypothesis_passed": mean_speedup >= PHI,  #   = 1.618
