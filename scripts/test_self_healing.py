...
async def run_self_healing_audit():
    ...
    try:
        ...
        if not analyst or not sage:
            print(
                "❌ CRITICAL: Required Dogs (ANALYST or SAGE) not found in the organism."
            )
            return
        ...
        judgments = await asyncio.gather(analyst.analyze(cell), sage.analyze(cell))
        ...
        consensus = await cynic_coordinator.phi_bft_run(
            cell, [analyst_judgment, sage_judgment]
        )
