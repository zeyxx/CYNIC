...
async def judge(req: JudgeRequest, container: AppContainer = Depends(get_app_container)) -> JudgeResponse:
    ...
    time_dim = req.time_dim or infer_time_dim(str(req.content), enriched_context, req.analysis)
...
async def learn(req: LearnRequest, container: AppContainer = Depends(get_app_container)) -> LearnResponse:
    ...
    return LearnResponse(
        state_key=req.state_key, action=req.action, q_value=0.5, visits=1, 
        confidence=0.0, wins=0, losses=0
    )

@router_core.post("/feedback", response_model=FeedbackResponse)
async def feedback(req: FeedbackRequest) -> FeedbackResponse:
    return FeedbackResponse(
        state_key="unknown", action="WAG", reward=float(req.rating)/5.0,
        q_value=0.5, visits=1, message="Feedback received"
    )

@router_core.get("/policy/{state_key}", response_model=PolicyResponse)
async def get_policy(state_key: str, container: AppContainer = Depends(get_app_container)) -> PolicyResponse:
    return PolicyResponse(
        state_key=state_key, mode="exploit", recommended_action="WAG",
        q_value=0.5, confidence=0.0, top_actions=[]
    )
