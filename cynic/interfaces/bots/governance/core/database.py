    ...
    async with aiohttp.ClientSession() as session:
        ...
        try:
            async with session.request(method, url, json=json) as resp:
                ...
                logger.error(
                    "Organism API Error (%d): %s", resp.status, await resp.text()
                )
                return None
...
async def update_proposal_status(*args, **kwargs):
    pass


async def update_proposal_judgment(*args, **kwargs):
    pass


async def count_votes(*args, **kwargs):
    return (0, 0, 0)


async def update_vote_counts(*args, **kwargs):
    pass


async def is_voting_active(*args, **kwargs):
    return True


async def check_voting_closed(*args, **kwargs):
    return False


async def get_user_vote(*args, **kwargs):
    return None


async def get_proposals_needing_outcome(*args, **kwargs):
    return []


async def create_learning_outcome(*args, **kwargs):
    pass


async def mark_outcome_determined(*args, **kwargs):
    pass


async def get_or_create_e_score(*args, **kwargs):
    return None


async def db_health_check():
    return {"status": "CONNECTED_TO_ORGANISM"}
...
async def update_e_score(*args, **kwargs):
    pass
