...
|------|---------|----------|---------|----------|--------|--------|-------|
| 2026-04-17 | r23-gate + lint-drift storage fix (K15 R21) | ~2h | 3 (1fa7d38 r23-gate, db3289b docs, 96099de lint-drift; PR #7/#8/#9 merged) | 0 | 2 (R23 meta-principle closed ; K15 silent-death root-caused + R21'd) | 0 | **Item 1 — R23 gate** : R23 adoptée 2026-04-16 sans gate = violation meta-principle + #20/#21. Pre-gate = 0 real violations (audit shell+Rust). Post-gate : négative control clean, 3 positive controls R23a/R23b/R23c catch, exempt marker pass. Scope : shell `cargo (build\|test\|clippy\|run\|check\|audit)` → `RUST_MIN_STACK` in file ; standalone `rustfmt` → `--edition` per line ; Rust `Command::new("cargo"\|"rustfmt")` → `.env(` / `"--edition"`. Wired into `make check` between lint-drift et lint-security. **Item 2 — lint-drift K15 silent death** : observé pendant item 1 (`grep: .../storage.rs: No such file`). S3 split (2026-04-16) déplaça `domain/storage.rs` → `domain/storage/mod.rs` ; Makefile lint-drift resté sur ancien path → 6 stores jamais itérés → K15 read-path check silencieusement mort. Preview avant fix : 6/6 stores ont read paths (no hidden violations). Fix : path update + K15 test-gates entry (inject phantom store → catch). **Mesure globale :** `make test-gates` = **17/17 falsifiés** (13 legacy + 3 R23 + 1 K15). Pre-commit validation clean x2. 3 PRs, 3 merges admin, tree clean, 0 unpushed. **Crystallisation :** r23 session a généré sa propre suite (patterns → item 2). **Pacing :** user-driven 2 items (R23, puis "on fait B"), stop. |
