...
logger = logging.getLogger("cynic.brain.llm.cli_bridge")
...
{request.prompt}" if request.system else request.prompt
        ...
        try:
            ...
            if process.returncode == 0:
                ...
                return LLMResponse(
                    ...
                    error=stderr.decode().strip(), latency_ms=latency_ms
                )
    ...
    async def check_available(self) -> bool:
        try:
            proc = await asyncio.create_subprocess_exec(self._binary, "--version", stdout=asyncio.subprocess.DEVNULL, stderr=asyncio.subprocess.DEVNULL)
