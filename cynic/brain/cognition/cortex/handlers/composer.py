...
Typical usage:
    from cynic.brain.cognition.cortex.handlers.composer import HandlerComposer
    composer = HandlerComposer(registry)
    result = await composer.compose(pipeline, level=ConsciousnessLevel.MACRO)

See Also:
    cynic.brain.cognition.cortex.handlers.base: BaseHandler contract
    cynic.brain.cognition.cortex.handlers.registry: HandlerRegistry
"""
...
if TYPE_CHECKING:
 from cynic.brain.cognition.cortex.orchestrator import JudgmentPipeline
logger = logging.getLogger("cynic.brain.cognition.cortex.handlers.composer")
