...
## Commit Hygiene (derived from 120-commit audit, 2026-04-30)

24. **TODO is not a commit.** `TODO.md` changes are folded into the feature commit they document. Never commit TODO.md alone — it's bookkeeping, not a unit of work. 11 standalone TODO commits found in 120-commit audit (9.2% noise). — pre-commit hook (planned)
25. **One problem, one commit.** If fixing a single problem takes >1 commit, squash before push. The git history records solutions, not the debugging journey. Seen: 5 CCM fixes in 8min (01:58→02:06), 5 K15 systemd fixes in 16min (01:45→02:01). Each cascade should have been 1 commit. `git rebase -i` before `git push` to collapse fix chains. Exception: commits spanning different files/concerns stay separate even if related in time.
26. **PR ≤ 20 files, ≤ 1 scope.** A PR touching >20 files or combining >1 independent feature requires decomposition before merge. Seen: PR#33 (53 files, 4 features), PR#44 (66 files), PR#48 (41 files). Scope creep during implementation → split the branch, don't expand the PR. — Falsify: `gh pr diff --stat | head -1` before merge; reject if file count > 20.
27. **`git add` names files, never globs at root.** `git add .` and `git add -A` at repo root are forbidden — they capture stray artifacts. Seen: PostScript files named `json`, `os`, `sys` accidentally committed (+157K lines, PR#49). Always `git add <specific-files>` or `git add <directory>/`. Root-level artifacts are gitignored defensively (`.gitignore` /json /os /sys *.ps).
