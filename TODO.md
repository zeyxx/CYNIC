...
- [ ] **Tier 2: Gemini meta-advisor deployment.** When API quota resets: deploy hermes-x-gemini-meta.service (cron post-organ-cycle). Reads last 5 reflections + feedback log, queries Gemini, stores META_GUIDANCE in SKILL.md. **Falsify:** Agent decisions shift toward Gemini recommendations within 2 cycles. **Blocked:** Gemini API quota (resets in ~11h from 04:15 UTC 2026-04-30).
- [ ] **Tier 3: Self-aware organism (trend detection).** proof_of_evolution.py runs 7-cycle window. Compute: domain_count growth rate, confidence_convergence trend, verdict_growth rate, robustness health%, anomaly_reduction slope. **Falsify:** ≥2 metrics show monotonic improvement → organism knows it's learning.
